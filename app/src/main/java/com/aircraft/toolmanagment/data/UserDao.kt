...
interface UserDao {
    ...
    @Query("SELECT * FROM User WHERE (name = :identifier OR employee_id = :identifier) AND password = :password")
    suspend fun getUserByIdentifierAndPassword(identifier: String, password: String): User?

    @Query("SELECT * FROM User WHERE name = :name OR employee_id = :userId LIMIT 1")
